<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Strict//EN">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Home | Weinhandel SW & DA</title>

<link href="style.css" rel="stylesheet" type="text/css" />
</head>

<body>
<div class="style3"></div><div class="style_2"><span class="style3"><a><strong></strong></a></span></div>
<div id="wrap">
	<!-- Start "HeaderBereich" -->
	<div id="topbar">
	  <h1 id="sitename"><a>Weinhandlung SW & DA</a></h1>
	  <div id="menus">
	  	<ul id="topmenu">
			<li>
				<h4><a href="index.jsp">Home</a></h4>
			</li>
			<li>
				<h4><a href="ueberuns.jsp">Über uns</a></h4>
			</li>
			<li>
				<h4><a href="bestellungen.jsp">Bestellungen</a></h4>
			</li>
			<li>
				<h4><a href="#">Mitarbeiterlogin</a></h4>
			</li>
		</ul>
	  </div>
	</div>
	<div id="header">
	<!-- Beinhaltet nur das Headerbild -->
	</div>
	<!-- Ende "Headerbereich" -->
	
	<!--  Beginn Content Bereich -->	
	<div id="content">
		<!--  Mainpage -->
		<div id="mainpage">
			<h2>Willkommen auf der Webseite vom SW & DA Weinhandel</h2>
			<p>
				Auf unserer Internetseite haben Sie die Möglichkeit, erstklassige Wein aus dem In- und Ausland zu bestellen.
				Bei weiteren Fragen schreiben Sie uns eine E-Mail support@weinhandelswda.com oder rufen Sie unsere Hotline an
				unter der Nummer 089-123456789876.
			</p>
		</div>
		<!-- Sidebar -->
		<div id="sidebar">
		
		</div>
		<!-- SidebarContent -->
		<div id="sidebarcontents"> 
			<a href="#">Warenkorb</a>
		</div>
	
	</div>
	<!-- Ende ContentBereich -->	

</div>

<div id="footer">
<p>Copyright &copy; 2014 | SW  DA Weinhandel</p>
</div>
</body>
</html>
